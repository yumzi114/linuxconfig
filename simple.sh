...
cd yay-bin
makepkg -si
yay -S gdb ninja gcc cmake meson libxcb xcb-proto xcb-util xcb-util-keysyms libxfixes libx11 libxcomposite xorg-xinput libxrender pixman wayland-protocols cairo pango seatd libxkbcommon xcb-util-wm xorg-xwayland libinput libliftoff libdisplay-info chromium base-devel
...
yay -S thunar thunar-archive-pluginfile-roller mpv geany viewnior mpc mpd ncmpcpp chromium neofetch sway
sudo cp -r usr /
sudo cp -r etc /
cp -r .config ~/
cp -r .local ~/
